...
public interface PeneiraRepository extends JpaRepository<Peneira, UUID> {
    ...
    @Query(value = """
        ...
        GROUP BY p.id
        """, nativeQuery = true)
    Optional<GetPeneiraProjection> findByPeneiraIdComUniformes(@Param("id") UUID id);
    ...
    @Query(value = """
        ...
        LEFT JOIN endereco AS e ON e.id = c.endereco_id
        LEFT JOIN peneira_uniform AS pu ON pu.peneira_id = p.id
        WHERE p.clube_id = :clubeId
        GROUP BY p.id, c.id, c.name, c.clube_img, e.cep
        """, nativeQuery = true)
    List<GetPeneiraProjection> findByClubeIdComUniformes(
    ...
    @Query(value = """
        ...
        GROUP BY p.id, c.id, c.name, c.clube_img, e.cep
        """, nativeQuery = true)
    List<GetPeneiraProjection> findByIdsComClubeEUniformes(
